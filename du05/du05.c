...
static
int
buf_new (buf_type * buf, size_t len)
{
    memset (buf, 0, sizeof (*buf));

    buf->len = len;
    buf->cap = BUF_FIXED_CAP;
...
buf_realloc (buf_type * buf, size_t new_cap)
{
    (void)buf;
    (void)new_cap;
    /* This version does not support reallocation.
       The buffer has fixed length. */
    exit (EXIT_FAILURE);
}
    ...
    size_t rows)
{
         ...
         ++haystack_c)
    {
        ...
        {
            int ret;
            char const * haystack_row = NULL;
            char const * haystack_row_it = NULL;
            char const * needle_row_it = NULL;

            haystack_row = buf_getdata_const (&haystack[row]);
            haystack_row_it = haystack_row + haystack_c;

            needle_row_it = buf_getdata_const (&needle[row]);

            assert (haystack_row_it - (haystack_row + haystack_columns) < 0);
